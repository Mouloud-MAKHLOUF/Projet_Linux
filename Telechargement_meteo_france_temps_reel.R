list.of.packages <- c("httr","glue","sf","rNOMADS","raster")
...
URL <- glue("https://public-api.meteofrance.fr/previnum/DPPaquetARPEGE/v1/models/ARPEGE/grids/0.1/packages/{nom_paquet}/productARP?referencetime={date_string}T{heure_prevision}%3A00%3A00Z&time={echeance}&format=grib2")

response <- GET(URL, add_headers(apikey = apikey))

#-- Chemin pour sauvegarder le fichier telechargé
nom_fichier <- glue("donnees_meteo_france/donnees_meteo_france_{nom_paquet}_{date_string}T_{heure_prevision}t_echeance_{echeance}.grib2")
...
if (http_status(response)$category == "Success") {
  ...
  writeBin(grib_content, nom_fichier)
...
grib_file <- nom_fichier
raster_data <- raster(grib_file)
